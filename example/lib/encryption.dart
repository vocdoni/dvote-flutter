...
class _EncryptionScreenState extends State<EncryptionScreen> {
  ...
  void initPlatformState() async {
    ...
    try {
      start = DateTime.now();
      // sync
      encrypted = SymmetricNative.encryptString(MESSAGE, PASSPHRASE);
      decrypted = SymmetricNative.decryptString(encrypted, PASSPHRASE);

      mid = DateTime.now();

      // async
      encrypted = await SymmetricNative.encryptStringAsync(MESSAGE, PASSPHRASE);
      decrypted =
          await SymmetricNative.decryptStringAsync(encrypted, PASSPHRASE);
  ...
  bool testPriorValues() {
    ...
    var decryptedMessage =
        SymmetricNative.decryptString(encryptedB64, passphrase);
    ...
    decryptedMessage = SymmetricNative.decryptString(encryptedB64, passphrase);
    if (decryptedMessage != expectedMessage) return false;
  ...
  bool testRandomValues() {
    ...
    for (String msg in messages) {
      final encryptedB64 = SymmetricNative.encryptString(msg, passphrase);
      var decryptedMessage =
          SymmetricNative.decryptString(encryptedB64, passphrase);
