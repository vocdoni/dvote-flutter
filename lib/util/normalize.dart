...
class Normalize {
  static _initialize() {
    // Parse diacritics map
    _defaultDiacriticsRemovalMap.forEach((entry) {
      final letters = entry["letters"];
      for (var i = 0; i < letters.length; i++) {
        diacriticsMap[letters[i]] = entry["base"];
      }
    });
  }

  static String removeDiacritics(String input) {
    if (diacriticsMap.isEmpty) _initialize();
    return input.replaceAllMapped(RegExp(r"([^\u0000-\u007E])"), (match) {
      return diacriticsMap[match.group(1)] ?? match.group(1);
    });
  }

  static const List<Map<String, String>> _defaultDiacriticsRemovalMap = [
    {
      'base': 'A',
      'letters':
